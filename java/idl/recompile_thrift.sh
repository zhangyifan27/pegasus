...
# 3. ./configure --with-csharp=no --with-java=no --with-python=no --with-erlang=no --with-perl=no --with-php=no --with-ruby=no --with-haskell=no --with-go=no --with-nodejs=no --with-lua=no --with-php_extension=no
