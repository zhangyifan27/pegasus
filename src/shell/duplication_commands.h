...
inline bool add_dup(command_executor *e, shell_context *sc, arguments args)
{
    ...
    if (err != dsn::ERR_OK) {
        fmt::print("adding duplication for app [{}] failed, error={}", app_name, err.to_string());
    }
    return true;
}
...
inline bool query_dup(command_executor *e, shell_context *sc, arguments args)
{
    ...
    if (err != dsn::ERR_OK) {
        fmt::print("querying duplications of app [{}] failed, error={}", app_name, err.to_string());
    }
    return true;
}
...
{
    auto result = dsn::replication::convert_str_to_dupid(str);
    if (!result.is_ok()) {
        fmt::print(stderr, "parsing {} as positive int failed: {}\n", str, result.get_error());
        return false;
    }
    *dup_id = result.get_value();
    return true;
}
