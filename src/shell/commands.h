...
{
    if (length == 9 && strncmp(buffer, "not_exist", 9) == 0) {
        result = pegasus::pegasus_client::CT_VALUE_NOT_EXIST;
        return true;
    }
    if (length == 5 && strncmp(buffer, "exist", 5) == 0) {
        result = pegasus::pegasus_client::CT_VALUE_EXIST;
        return true;
    }
    if (length == 9 && strncmp(buffer, "not_empty", 9) == 0) {
        result = pegasus::pegasus_client::CT_VALUE_NOT_EMPTY;
        return true;
    }
    if (length == 5 && strncmp(buffer, "equal", 5) == 0) {
        result = pegasus::pegasus_client::CT_VALUE_EQUAL;
        return true;
    }
    if (length == 14 && strncmp(buffer, "match_anywhere", 14) == 0) {
        result = pegasus::pegasus_client::CT_VALUE_MATCH_ANYWHERE;
        return true;
    }
    if (length == 12 && strncmp(buffer, "match_prefix", 12) == 0) {
        result = pegasus::pegasus_client::CT_VALUE_MATCH_PREFIX;
        return true;
    }
    if (length == 13 && strncmp(buffer, "match_postfix", 13) == 0) {
